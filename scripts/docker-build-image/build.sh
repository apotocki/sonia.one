...
apt-get update
apt-get install -y gcc-10 g++-10 gdb valgrind ssh git cmake libicu-dev zlib1g-dev libbz2-dev openssl libssl-dev default-jre
#clang-7 clang-tools-7 libclang-common-7-dev libclang-7-dev libclang1-7 clang-format-7 libc++-7-dev libc++abi-7-dev
...
./bootstrap.sh
./b2 install -j4 -a --toolset=gcc-10 release cxxflags="-std=c++17 -fPIC" address-model=64 link=shared,static runtime-link=shared --layout=versioned define=BOOST_TEST_NO_MAIN define=FUSION_MAX_VECTOR_SIZE=12 define=BOOST_FUSION_INVOKE_MAX_ARITY=12 define=BOOST_SPIRIT_THREADSAFE --with-atomic --with-context --with-chrono --with-exception --with-date_time --with-thread --with-program_options --with-regex --with-test --with-system --with-log --with-serialization --with-graph --with-filesystem --with-locale --with-random --with-stacktrace --disable-filesystem2
