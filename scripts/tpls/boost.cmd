...
bjam.exe -j8 include="%WIN_KIT_INCLUDE%" linkflags="/LIBPATH:\"%WIN_KIT_LIB%\"" -sICU_PATH="%TPLS_HOME%\icu" -sICU_LINK="-LIBPATH:%TPLS_HOME%\icu\lib icuuc.lib icuin.lib icudt.lib" toolset=msvc-14.1 release debug address-model=64 architecture=x86 define=_CRT_NONSTDC_NO_DEPRECATE define=_CRT_SECURE_NO_WARNINGS define=_SCL_SECURE_NO_WARNINGS define=_ITERATOR_DEBUG_LEVEL=0 define=BOOST_SPIRIT_THREADSAFE -sICU_PATH="%TPLS_HOME%\icu" --with-date_time --with-thread --with-chrono --with-program_options --with-regex --with-test --with-system --with-log --with-serialization --with-graph --with-filesystem --with-random --with-locale --with-context --with-fiber debug-symbols=on link=shared runtime-link=shared
